...
import { ArrowLeft, ChevronRight, Bookmark, User, Calendar } from "lucide-react";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  try {
    ...
    if (!response.items.length) {
      return { title: "Blog Not Found | High Rise Digital" };
    }

    const fields: any = response.items[0].fields;

    return {
      title: fields.metaTitle || fields.title || "Latest Insight | High Rise Digital",
      description: fields.metaDescription || "Read our premium framework deployment details inside.",
    };
  } catch (error) {
    return { title: "Blog | High Rise Digital" };
  }
}
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <article className="bg-[#030712] border border-white/5 p-6 md:p-10 rounded-[2.5rem] shadow-2xl relative overflow-hidden">
              ...
              <div className="relative z-10 space-y-8">
                <p className="text-white text-[16px] md:text-base lg:text-[17px] 2xl:text-xl font-medium leading-relaxed border-l-4 border-blue-500 pl-6 py-1 max-w-full">
                  {post.excerpt}
                </p>
