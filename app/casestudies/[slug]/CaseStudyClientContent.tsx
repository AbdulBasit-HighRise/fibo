...
export default function CaseStudyClientContent({ project }: CaseStudyProps) {
  ...
  return (
    <main className={`bg-[#030303] text-zinc-100 min-h-screen antialiased overflow-x-hidden pt-0 transition-opacity duration-1000 ${isSocial ? "selection:bg-pink-600" : "selection:bg-blue-600"
      }`}>
        ...
        <div className="w-full">
          ...
          <div className="relative z-10 w-full mx-auto text-center lg:text-left px-6 pt-32 pb-16 md:pt-20 md:pb-24 lg:max-w-full lg:pt-40 lg:pb-16 lg:pl-40 2xl:mx-0 2xl:max-w-[1500px] 2xl:pt-48 2xl:pl-80">
            <div className="max-w-5xl animate-fadeIn space-y-2">
              ...
              <div className={`flex items-center gap-2.5 mb-8 w-fit px-4 py-2 rounded-full border backdrop-blur-md mx-auto lg:mx-0 ${isSocial ? 'bg-pink-500/10 border-pink-500/20 text-pink-300' : 'bg-blue-500/10 border-blue-500/20 text-blue-300'
                }`}>
              ...
              <h1 className="text-[3rem] md:text-[3.2rem] lg:text-[3.8rem] 2xl:text-[5rem] font-black tracking-tighter leading-none text-white mb-8 uppercase">
                CASE{" "}
                <span className={`bg-gradient-to-r bg-[length:200%_auto] animate-gradient text-transparent bg-clip-text ${isSocial ? 'from-pink-400 via-rose-500 to-pink-400' : 'from-[#00f2ff] via-[#0070ff] to-[#00f2ff]'
                  }`}>
                  STUDIES.
                </span>
              </h1>
      ...
      <div className="w-full">
        {project.sections.map((section, idx) => {
          ...
          return (
            <section
              key={section.id}
              className={`w-full py-24 border-b border-white/[0.02] transition-all duration-700 ${isDarkSlate ? 'bg-[#020617]' : 'bg-[#030303]'
                }`}
            >
                    ...
                    <div className="space-y-4 pt-4">
                      {section.features.map((feat, fIdx) => (
                        <div key={fIdx} className="flex items-start gap-3 group py-1.5 border-b border-white/[0.02] hover:border-white/[0.08] transition-colors duration-300">
                          <div className={`mt-2 w-1.5 h-1.5 rounded-full shrink-0 ${isSocial ? 'bg-pink-500 shadow-[0_0_8px_rgba(236,72,153,0.6)]' : 'bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.6)]'
                            }`} />
