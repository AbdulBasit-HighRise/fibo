     ...
     px-4 sm:px-6 md:px-8 lg:px-16 2xl:px-24 py-10 md:py-14 2xl:py-20">
      ...
      <div className="relative z-10 max-w-7xl 2xl:max-w-[110rem] mx-auto flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8 lg:gap-12 2xl:gap-16">
        ...
        <div className="flex flex-nowrap items-center justify-start lg:justify-start gap-3 sm:gap-6 md:gap-8 2xl:gap-12 flex-1 w-full lg:w-auto shrink-0">
          ...
          {/* PHONE / CALL OPTION */}
          <Link href="https://wa.me/923002777587" target="_blank" className="group flex items-center gap-2 sm:gap-4 2xl:gap-6 shrink-0">
            <div className="w-9 h-9 sm:w-12 sm:h-12 2xl:w-16 2xl:h-16 rounded-full border border-white/10 bg-white/5 flex items-center justify-center group-hover:border-green-500/50 group-hover:bg-green-500/10 transition-all duration-500">
              {/* Yahan Phone ki jagah MessageSquare ya agar Lucide-react mein MessageCircle hai to woh use kar lo, ya direct text/svg daal do */}
              <MessageSquare className="w-3.5 h-3.5 text-green-400 sm:w-5 sm:h-5 2xl:w-6 2xl:h-6 group-hover:scale-110 transition-transform" />
            </div>
            <div className="flex flex-col">
              <span className="text-[8px] sm:text-[10px] 2xl:text-xs text-white font-bold uppercase tracking-widest leading-none mb-1 2xl:mb-1.5">Chat on WhatsApp</span>
              <span className="text-xs sm:text-base md:text-lg 2xl:text-2xl text-white font-black tracking-tight group-hover:text-green-400 transition-colors">
                +92 300-2777587
              </span>
            </div>
          </Link>
